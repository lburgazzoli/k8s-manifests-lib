...
func TestTransformer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts: []jq.Option{
				jq.WithFunction("addPrefixToLabels", 1, 1, func(input any, args []any) any {
		...
		},
		{
			...
			opts: []jq.Option{
				jq.WithVariable("greeting", "Hello, World!"),
			},
			inputObject: &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-cm",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			transformer, err := NewTransformer(tt.expression, tt.opts...)
			if tt.expectNewErr {
			...
			transformed, err := transformer(context.Background(), unstrObj)
