...
func TestRenderer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			objects: []runtime.Object{pod, configMap},
			opts: []mem.Option{
				mem.WithFilter(gvk.NewFilter(corev1.SchemeGroupVersion.WithKind("Pod"))),
			},
			expectedCount: 1,
			validation: And(
				jqmatcher.Match(`.kind == "Pod"`),
		...
		},
		{
			...
			objects: []runtime.Object{pod},
			opts: []mem.Option{
				mem.WithTransformer(labels.NewTransformer(map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			renderer := mem.NewRenderer(unstructuredObjects, tt.opts...)
